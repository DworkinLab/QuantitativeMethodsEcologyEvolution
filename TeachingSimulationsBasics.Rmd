...
Let us simulate 100 observations from a normally distributed variable with a mean of 5 and a standard deviation of 2  $\sim N(\mu = 5, \sigma = 2)$


We will use the `rnorm` function which generates **r**andom values from a **norm**al distribution

```{r}
norm_simulated <- rnorm(n = 100, mean  = 5, sd = 2)

par(mfrow=c(1,1))

plot(norm_simulated, pch = 20)

hist(norm_simulated, freq = F)
...
We can ask about the spread of the means from our simulated trials. How might you code this?
...
sd(apply(norm_sim_all_5, 2, mean)) # n=1000
```
...
Instead of simulating just a mean and sd, let's specify something more interesting... How about a line/regression!!!!!
...
par(mfrow = c(2, 1))
plot(y_fixed ~ x, pch = 20, col = "red")  # The deterministic relationship.
```
...
`length` just looks at the number of elements in the vector. So we have now simulated values of our response variable `y` given a deterministic and stochastic component to the model!


```{r}
plot(y.sim.1 ~ x, pch = 20, col = "red")
abline(a = 5, b = 0.7, lwd = 2) # Expected relationship based on the "known" parameters we used.
```

This is the expected relationship based on the "known" parameters we used.

But what are the actual parameter estimates for the regression based on the simulated data (for y)? We can use the `lm` function to run a linear regression.
...
confint(y.sim.1.lm) # does it include our expected (TRUE) values?

plot(y.sim.1 ~ x, pch = 20, col = "red")

abline(a = 5, b = 0.7, lwd = 2) 
abline(reg=y.sim.1.lm, lty = 2, col = "purple", lwd = 2) # estimated values based on simulated data.
```
