...
# Do we ignore the variation among individuals (complete pooling)

ggplot(sleepstudy, aes(y = Reaction, x = Days)) + 
  geom_point() + geom_smooth(method = "lm")

# Do we fit a seperate regression for each individual?
# Do we ignore the variation among individuals (complete pooling)

ggplot(sleepstudy, aes(y = Reaction, x = Days, colour = Subject)) + 
  geom_point() + geom_smooth(method = "lm", alpha = 0.25, lty = 2)


ggplot(sleepstudy, aes(y = Reaction, x = Days, colour = Subject)) + 
  geom_point() + geom_smooth(method = "lm", alpha = 0.25, lty = 2) + facet_wrap(~Subject)
...
plot(Reaction ~ Days, 
     pch = 20, col = rgb(0, 0, 1, 0.25),
    data=sleepstudy, 
    main="Reaction time as a function of sleep deprivation")

lm.pooled <- lm(Reaction ~ 1 + Days, data  =sleepstudy)

summary(lm.pooled)

abline(lm.pooled, 
       lwd = 3, lty = 2, col = rgb(1, 0, 0, 0.5))
...
simmie <-function(){ # function to generate one simulation, and then plot the fitted line
    ...
    abline(a=sim.lm@coef[,1], b=sim.lm@coef[,2], 
           lty = 2,
           col = rgb(1, 0,0, 0.25)) }
...
#  We want to fit random effects for each individual for both their "intercept" (reaction time prior to sleep deprivation), 
#  As well as for the slope (rate at which their reaction time gets worse)

fm1 <- lmer(Reaction ~ 1 + Days + (1 + Days | Subject), 
            sleepstudy)
...
confint(fm1)
# Add the fit of the mixed model to the plot above
