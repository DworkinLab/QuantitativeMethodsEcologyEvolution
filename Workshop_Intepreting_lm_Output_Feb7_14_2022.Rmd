...
You will need to [download](https://www.dropbox.com/s/b9o0jqdhzc8suod/dll_data.csv?dl=0) it first. Put it in the data folder.


Local copy
 
```{r}
sct_data  <- read.csv("../data/dll.csv",
...
## We are not going to do any major "data checks" today
 ...
 What we always should start with is some EDC (Exploratory Data Checks). Sometimes called EDA (analysis). i.e. to look for outliers (and check to see if they are real data points, transcription errors etc), accidential data duplications etc...
...
It is important to start by examining the range over which the data occurs. We can do this in tabular form, and with simple plots. This will help us as we interpret the amount of change we observe in both predictor and response.
...
quantile(sct_data$tarsus, probs = c(0, 0.025, 0.5, 0.975, 1))
```

## Fitting the model as is

Let's go ahead and fit our very simple model, a *linear regression* of SCT number (our response) onto the length of the tarsus (our predictor).
...
What are these telling us? Interpret the intercept and the coefficient (fill in the word  *xxx* of the line...). How easy is this to interpret? Why?
...
So an increase of 26.9 SCT is greater than the entire range of variation we see for SCT. So this is clearly challenging to interpret.
...
Looking at the plot it should be clear that the range of tarsus lengths spans from about 0.1 mm to about 0.25mm. So only about 0.15mm in total. So a *unit* (1) change in mm is about 6.7 times larger than our entire range of variation for tarsus length.
...
Since slopes are per unit change, why don't we use units that are more sensible?
...
So the maximum effect in SCT number due to changes in the length of tarsus (from smallest to largest) is a bit bigger than 2 SD.
...
This relationship accounts for about 8.7% of the variation in  SCT. More on this later.
...
The fairly obvious issue (once you stare at the plot) is to realize that the intercept means the predicted number of SCT when the length of the tarsus is 0. That hopefully is a very weird notion. How can you have 6 SCT on something with zero length?

More generally this is a problem of extrapolating outside of the range of data. This has all sorts of issues (some small, some severe) that it can cause. But it has some very easy solutions.

So how might you "change" something to help make the intercept easier to interpret?

There are a couple of options that are sensible. One would be to subtract the smallest value of tarsus length from all observations. So then the intercept would represent the predicted number of SCT at the smallest leg in the sample.

However the common approach is to "mean center" the predictors (in this case the tarsus length). This means just compute the mean value for the tarsus length and subtract that from all values for the tarsus.
...
This is (for a number of reasons) the most sensible approach (especially with interactions, coming up later).

All this has done is move the whole distribution over:

```{r}
par(mfrow = c(1, 2))
plot(density(sct_data$tarsus_microM), main = "", col = "blue")
plot(density(sct_data$tarsus_microM_centered), main = "", col = "red")
par(mfrow = c(1, 1))
```

But otherwise no changes.

How does this help? Let's take a look by using this in the model:
...
You will notice the slope has not changed, only the intercept. The new value (`r coef(model3)[1]`) represents the predicted number of SCT at the mean value of tarsus, 188 $\mu m$. This value (188 $\mu m$) is now represented as our 0 value for this predictor. So for a fly with the average tarsus length (188 $\mu m$) it is predicted to have about 11.13 SCT (so 11).
...
So with just a few simple changes (changing the units and centering our predictor), we can make a lot more sense about our model output. Awesome.
...
FI am purposefully NOT fitting a linear model yet (i.e. standard slope of regression) in the plot below since that is what we are working towards. So we know we are going to fit the model below, but let's just get a sense of where we are going with it.
...
### We have seen this before by just examining the differences between the means
...
Now something pretty similar but using the effects librarys. Pretty much the same, just reporting slightly differently. Once we get to more complex models, the use of each package will become apparent (but probably emmeans is the one to focus on learning. Really great)
...
We now have a new row in our model for genotype. What do each of these coefficients mean?
...
One issue we have is that our units for tarsus are in $\mu m$. But genotype is a factor (mutant or wild-type or for the design matrix 0/1). So comparing the coefficients for each is apples to oranges. Depending on the model, this may not matter. But often it does. So what do you do if you need to compare them?

### Z-transformations

 You have probably heard in your stats class about Z transformations. This is a way to help make your different predictors a bit more comparable. We have already done the first step by mean centering the predictor. You can also then *scale* the variable by its standard deviation. That way a unit change is in units of SD of the variable. Better yet this will be the case for any variable.
...
This will mostly be discussed on February 17th, 2022.
