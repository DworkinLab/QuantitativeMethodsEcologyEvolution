...
tapply(resid(linear_model_2), 
    INDEX=list(dll.data$genotype, dll.data$temp), sd)
...
# use estimates from lm as starting values
coef(linear_model_2)

glm_MLE_fit_1 <- mle2(glm_MLE_support, 
            start=list(b0 = 10.8, b1 = 18.5, b2 = 1.2, 
            b3 = 0.09, b4 = -1.27, s0 =SE(dll.data$SCT), s1 = 0 ), method="BFGS")
...
ParBootMLE <- function(Model1 = glm_MLE_fit_1, RestrictedModel = glm_MLE_fit_2, X = design_matrix){
	...
	glm_MLE_support_restr <- function(b0 , b1, b2, b3, b4, y = simulated_response, sigma ) { 
	    	deterministic_part <- b0*X[,1] + b1*X[, 2] + b2*X[, 3] + b3*X[,4] + b4*X[,5]
            random_part <- sigma
           -sum(dnorm( y, mean = deterministic_part, sd =  random_part, log=T))}

    # support function for full model
   glm_MLE_support_full <- function(b0, b1, b2, b3, b4,s0, s1, y = simulated_response) {
     	deterministic_part <- b0*X[,1] + b1*X[, 2] + b2*X[, 3] + b3*X[,4] + b4*X[,5]
        random_part <- s0*X[,1] + s1*X[,3]
        -sum(dnorm( y, mean = deterministic_part, sd =  random_part, log=T))}
