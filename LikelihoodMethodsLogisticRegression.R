...
# Last updated December November 22nd 2011. HAH!!
...
crap1 <- sapply(vals, invlogit)
crap2 <- plogis(vals)
plot(y = crap1, x = vals)
plot(y = crap2, x = vals, col = "red")
####################
...
ggplot(dead.survived, aes(y = surv, x = size.std, col = Sex)) + 
    geom_jitter(height = 0.05, alpha = 0.24) + 
    geom_smooth()
...
# Negative log Likelihood calculator (Support function/objective function)
...
log.reg.optim.1 <- mle2(logistic.reg.fn.1, 
                        start=list(a = 0.5,b = 0.1))

log.reg.optim.2 <- mle2(logistic.reg.fn.2, 
                        start=list(a=0.5,b=0.1))

log.reg.optim.3 <- mle2(logistic.reg.fn.3, 
                        start=list(a=0.5,b=0.1))
...
coef(log.reg.optim.1)

plogis(coef(log.reg.optim.1)[1]) # probability of surviving at mean wing size (intercept)

plogis(sum(coef(log.reg.optim.1))) # probability of surviving at +1SD above mean wing size (intercept)

plogis(confint(profile.log.reg))
...
selection.glm <- glm(surv ~ size.std, 
                     family = binomial(link = "logit"),
                     data = dead.survived)
                     
coef(selection.glm)   # logit scale                  
plogis(coef(selection.glm)[1]) # Probability of survival at mean wing size (the intercept)
plogis(sum(coef(selection.glm))) # Probability of survival at + 1SD (i.e. a unit increase in wing size) above mean wing size

# If you look in ?glm or ?family you will see what choices you have for the glm(). 

summary(selection.glm)
confint(selection.glm)
...
par(mfrow=c(1,2))
curve( plogis(coef(selection.glm)[1] + coef(selection.glm)[2]*x), 
    -30, 30, ylab= " Survival probability")

curve( invlogit(coef(selection.glm)[1] + coef(selection.glm)[2]*x), 
    -60, 60, ylab= " Survival probability")
...
summary(selection.glm.null)
anova(selection.glm.null, selection.glm, test="Chisq")
AICctab(selection.glm.null, selection.glm, nobs=length(y), weights=T)
...
selection.lm.1 <- glm(surv ~ size.std,
     family = gaussian, data = dead.survived) # Wait why not use lm()?
 # no reason, just demonstrating that you can do it with glm

# Same model with lm
selection.lm.2 <- lm(surv ~ size.std, 
    data = dead.survived)
...
# almost identical fits.. Not surprising for these very small parameters.
 ...
 #####  Let's look at the more realistic model including quadratic
 selection.glm.quad <- glm(surv ~ Sex + size.std + Sex:size.std + I(size.std^2),
                           family = binomial(link="logit"), 
                           data = dead.survived)
 
 summary(selection.glm.quad)
 
 # this is a good use for emtrends in emmeans
 
emtrends( selection.glm.quad , specs = ~ Sex, var =  "size.std")

summary(emtrends( selection.glm.quad , specs = ~ Sex, var =  "size.std"))

summary(emmeans( selection.glm.quad , specs = ~ Sex), type = "link")
summary(emmeans( selection.glm.quad , specs = ~ Sex), type = "response")
 
 AICctab(selection.glm,selection.glm.quad, selection.glm.null, 
          nobs=length(y), weights=T)
...
coda::HPDinterval(bayes.logistic.1)
