# Written by Ian Dworkin - last modified on March 2nd, 2022
...
BootstrapMean <- function(X = dll.data$SCT){
	#x.boot <- sample(X, size = length(X), replace = T) # works for this simple example
	x.boot <- X[sample(length(X), size = length(X), replace = T)] # more general way
	mean(x.boot) }


N <- 300000 # How many iterations we are doing.
...
system.time(for (i in 1:N){  stat[i] <- BootstrapMean()})

# quick plot from both replicate and for loop
par(mfrow=c(1,2))    
hist(stat)
abline(v = mean(dll.data$SCT), 
       lwd = 2, col = "red")
...
# parametric confidence intervals based on assumptions of normality (we could also use the t distribution)
...
quantile(stat, 
         probs = c(0.025, 0.975)) # empirically determined confidence intervals

quantile(boot.replicate, 
         probs = c(0.025, 0.975))
...
text(0.85,920,"Distribution of bootstrapped differences",
     offset = ?0, cex = 1.3, col= "red")
