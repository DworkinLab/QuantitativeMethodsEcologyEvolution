...
sample, random sample of the overall population
...
```{r}
EstimatesLawLargeNumbers <- function(x, mn = 20, stdev = 6){
	# generate random numbers from the distribution
	sample_vals <- rnorm(n=x, mean = mn, sd = stdev)
...
```{r}
samples <- t(sapply(sample_size, 
                    EstimatesLawLargeNumbers, 
                    simplify = T))
```
...
```{r}
samples_sd15 <- t(sapply(sample_size, 
                    EstimatesLawLargeNumbers, stdev = 15,
                    simplify = T))
```
...
plot(samples_sd15 [,2] ~ samples_sd15 [,1], 
     main= "Sample Mean VS sample size", 
    ylab= "sample mean", xlab=" sample size", pch=1)
abline(h=20, lwd=3, col="red") # generates a horizontal line

plot(samples_sd15 [,3] ~ samples_sd15 [,1], 
     main= "Sample sd VS sample size", 
    ylab= "sample sd", xlab=" sample size", pch=1 )
abline(h=15, lwd= 3, col="red")

plot(samples_sd15 [,4] ~ samples_sd15 [,1], 
     main= "Sample cv VS sample size", 
    ylab= "sample cv", xlab=" sample size", pch=1 )
abline(h=0.75, lwd= 3, col="red")

plot(samples_sd15 [,5] ~ samples_sd15 [,1], 
